...
- the build is not certified (return to `gaia-product-compliance-officer`, S7)
- the title is already scale-ready and only needs the GTM engine (use `gaia-product-gtm-launch`)
...
6. Tune economy + onboarding, then prove each change with ONE-VARIABLE A/B; loop validated prices back to `gaia-product-monetization-economist` (S3).
...
- loop validated prices/economy back to `gaia-product-monetization-economist` (S3)
