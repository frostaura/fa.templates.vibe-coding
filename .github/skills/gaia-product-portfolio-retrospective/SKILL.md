---
name: gaia-product-portfolio-retrospective
...
- the title is healthy and only needs day-to-day operations (use `gaia-product-liveops`)
- a new bet is being framed from scratch with no prior priors (use `gaia-product-discovery`, S1)
...
- on a GO follow-on, hand priors + payer base + tooling to `gaia-product-market-researcher` (S1)
