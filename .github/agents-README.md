...
- The **Workload Coordinator** is the supreme planner and owns the MCP task graph.
- The **Quality Gatekeeper** has veto power.
- The **Repo Explorer** runs first on every request.

## Agents

- **gaia-workload-coordinator** — Supreme planner/controller: runs Repo Explorer first, fixes blockers first, creates MCP tasks + gates, delegates, enforces QA veto, records MCP proof.
...
1. Coordinator delegates to Repo Explorer (always).
2. Coordinator creates the task graph (MCP) and sets `required_gates[]`.
3. Architect/Developer/Tester execute tasks using Skills.
4. Quality Gatekeeper reviews every task and can veto.
5. Coordinator marks completion via MCP with link-only proof args.
