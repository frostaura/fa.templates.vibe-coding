...
- `gaia-product-process` on every run (the lifecycle and handoff contract)
- `gaia-unit-economics-model` to initialize v0 in Discovery and re-run at every gate
- `gaia-product-money-gate` at every stage boundary
...
- Evaluate the gate via `gaia-product-money-gate`: PASS → advance; FAIL on a fixable lever → loop-back; uneconomic after honest net math → kill.
...
- S1 → `gaia-product-market-researcher`; S2 → `gaia-product-ideation-strategist`; S3 → `gaia-product-monetization-economist`; S4 → `gaia-product-validation-researcher`; S5–S6 → `gaia-product-build-architect`; S7 → `gaia-product-compliance-officer`; S8–S9 → `gaia-product-growth-marketer`; S10 → `gaia-product-liveops-manager`.
- Review: launch ≥2 `gaia-product-reviewer` instances independently per stage.
- Synthesis: `gaia-product-synthesizer` merges specialist outputs and reviewer corrections.
- Parallel-safe within a stage; never parallel across a causal gate.
- When a validated bet needs construction, hand off to `gaia-solutions-architect` (delivery chain).
...
|---|---|---|
| weak / no paid demand (S4) | interest without money | `gaia-product-ideation-strategist` (re-select) or kill |
| WTP below ladder economics (S4) | price model unviable | `gaia-product-monetization-economist` (re-tune) |
| CPI > net LTV (S4/S8) | acquisition uneconomic | `gaia-product-monetization-economist` + model update |
| high-retention / zero-conversion (S8) | engagement toy, no business | `gaia-product-ideation-strategist` (reopen monetization verdict) |
| store-CVR failure (S9) | listing cannot convert | `gaia-product-growth-marketer` (creative / positioning) |
| churn = monetization pressure (S10) | economy over-extracts | `gaia-product-monetization-economist` |
