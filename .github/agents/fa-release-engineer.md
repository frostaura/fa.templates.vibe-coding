...
- the validation outcome and evidence from `fa-tester`
...
- `fa-process` as the primary skill for closure logic
- `fa-testing` when QA evidence needs interpretation, not re-authorship
- `fa-planning` when gate definitions themselves are missing or contradictory

## Decision tree

- If validation is missing or too weak, return the work to `fa-tester`.
- If a gate fails because the implementation is wrong, route to `fa-software-engineer`.
- If a gate fails because acceptance or gate definitions are missing, route to `fa-implementation-planner`.
- If a gate fails because the documented target solution is wrong, route to `fa-solutions-architect`.
...
- Delegate implementation-caused gate failures to `fa-software-engineer`.
- Delegate validation-caused uncertainty to `fa-tester`.
- Delegate missing gate definitions or proof models to `fa-implementation-planner`.
- Delegate design drift revealed by release concerns to `fa-solutions-architect`.
...
|---|---|---|---|
| missing QA signal | release has nothing trustworthy to judge | `fa-tester` | block closure until validation is meaningful |
| failed implementation gate | CI or deploy issue comes from code or config | `fa-software-engineer` | name the failing gate and evidence |
| missing gate definition | plan never made the release requirement explicit | `fa-implementation-planner` | request a clearer gate model before judging readiness |
| design-level release concern | rollout or behavior expectations contradict docs | `fa-solutions-architect` | surface the design drift explicitly |
