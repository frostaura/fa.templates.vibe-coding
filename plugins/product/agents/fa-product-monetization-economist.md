...
| auto-renew item | recurring charge framed as one-off | `fa-product-coordinator` (scope decision) |
| LTV:CAC < 3:1 | acquisition does not pay back | `fa-product-coordinator` (fail, name lever) |
| gross-only economics | fees / refunds / tax not netted | self-correct, re-derive net |
| whale concentration | revenue fragile to a few payers | `fa-product-coordinator` (flag fragility) |
