...
- If the build is uncertified, return it to `fa-product-coordinator`.
...
- Return growth artifacts to `fa-product-coordinator`; never call peer specialists directly.
- Loop-backs routed via the coordinator: `fa-product-ideation-strategist` (engagement-without-spend), `fa-product-monetization-economist` (CPI > net LTV or price re-validation).
...
|---|---|---|
| high-retention / zero-conversion | engagement toy, no business | `fa-product-ideation-strategist` (reopen monetization verdict) |
| CPI > net LTV | acquisition uneconomic | `fa-product-monetization-economist` (re-tune) + model update |
| validated prices still break LTV | price model unviable at scale | `fa-product-monetization-economist` (re-tune) |
