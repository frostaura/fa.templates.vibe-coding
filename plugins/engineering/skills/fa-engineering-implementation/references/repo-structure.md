...
Use a role-based structure instead of a web-biased `frontend` and `backend` split. The top level separates runnable applications (`apps/*`) from reusable libraries (`packages/*`), then organizes each project by responsibility.
...
- `apps` - runnable or deployable entry points, one folder per app role
...
- `packages` - reusable libraries shared across apps, organized by architectural layer
  - `presentation` - shared UI components and delivery-layer code
  - `application` - use-case coordination and workflow logic
  - `domain` - business meaning, entities, and invariants
  - `infrastructure` - persistence, external services, and technology concerns
...
Not every repo needs every folder. Use only the roles the project actually has.
...
- `src` - source code if the ecosystem expects a source folder
  - `components` - reusable UI components when the project has a visual surface
    ...
    - `organisms` - larger sections composed of molecules and atoms that form distinct regions of a page, such as a header, sidebar, or data table with its controls
...
## Migration Guidance: Legacy `src/` Layouts

When standardizing a repo that still uses the older `src/frontend` and `src/backend` split, map the old folders onto the current model instead of preserving both generations:

- `src/frontend` -> `apps/web`, or another app role such as `apps/mobile` or `apps/desktop`
- `src/backend/<main api project>` -> `apps/api`
- `src/backend/<core project>` -> `packages/domain`, and sometimes `packages/application`
- `src/backend/<data project>` -> `packages/infrastructure` or `packages/infrastructure/persistence`

Migrate incrementally, but do not leave a repo advertising both structures: the target is always `apps/*` + `packages/*`.
