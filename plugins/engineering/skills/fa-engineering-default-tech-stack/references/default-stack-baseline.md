...
- Start with the Tailwind theme configuration and global CSS so token and primitive foundations land first — the CSS-first `@theme` block on Tailwind CSS v4, or `tailwind.config.js` on a v3 codebase.
...
## MCP surface baseline (current practice, verified 2026-08)

- Build on the official `ModelContextProtocol` C# SDK at its **current major (2.x)** — it targets MCP spec revision **2026-07-28** (stateless protocol, discovery, MRTR) with automatic back-compat for legacy `initialize` clients.
- Serve Streamable HTTP; when tools need server-to-client interaction with legacy clients, set the hybrid session mode (`HttpServerSessionMode.StatefulForInitializeClients`) explicitly — the SDK default is stateless.
- Declare **tool annotations** (`readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`) on every tool, and enable **structured output** (`UseStructuredContent = true`) on tools with typed returns so clients get an `outputSchema`.
- Long-running tools report progress by taking an `IProgress<ProgressNotificationValue>` parameter — the SDK plumbs `notifications/progress` automatically when the caller supplies a progress token.
- Prefer **MRTR** (`input_required` / retry-with-responses) over the spec-deprecated server-initiated sampling, roots, and logging features; keep any deprecated usage isolated and documented.
- `ping` no longer exists in the current spec revision — health probes should POST a legacy `initialize` instead.
- In stdio mode, reserve stdout for the protocol and send diagnostics to stderr; never mute logging outright.
