...
- `fa-process` on every request
- `fa-default-tech-stack` when the user leaves stack choice open or asks for Gaia's standard baseline
- `fa-agents` when the problem is agent-definition drift or overlap
- `fa-skills` when the problem is skill-definition drift or reuse gaps
...
- If architecture or workflow docs are stale, route to `fa-solutions-architect`.
- If architecture is current but execution order is missing, route to `fa-implementation-planner`.
- If a bounded implementation task is already planned, route to `fa-software-engineer`.
- If the next question is validation quality, route to `fa-tester`.
- If the only remaining issue is gate readiness or proof, route to `fa-release-engineer`.
...
- Delegate design and documentation drift directly to `fa-solutions-architect`.
- Delegate plan publication directly to `fa-implementation-planner` after design is current.
...
| design drift | docs and intended behavior diverge | `fa-solutions-architect` | block planning and delivery until architecture is current |
| sequencing gap | target solution exists but execution order is missing | `fa-implementation-planner` | pass constraints, dependencies, and success criteria |
...
- **Not a fit:** "The plan is current and the branch just needs code changes." Send the work directly to `fa-software-engineer`.
