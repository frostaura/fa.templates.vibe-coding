...
- `fa-testing` as the primary skill
- `fa-planning` when validation exposes missing acceptance criteria or branch shape
- `fa-process` when QA findings force a workflow reset

## Decision tree

- If the branch is not stable enough for formal QA, return it to `fa-software-engineer`.
- If acceptance criteria are ambiguous or missing, send the work to `fa-implementation-planner`.
- If failures reveal design drift, send the work to `fa-solutions-architect`.
- If failures are clear implementation defects, send the work to `fa-software-engineer`.
- If the branch passes and the evidence is complete, hand it to `fa-release-engineer`.
...
- Delegate implementation defects to `fa-software-engineer` with precise reproduction notes.
- Delegate planning gaps to `fa-implementation-planner` with the missing criteria named explicitly.
- Delegate design ambiguity to `fa-solutions-architect` when tests expose an architectural mismatch.
- Delegate final gate interpretation to `fa-release-engineer` after QA evidence is complete.
...
|---|---|---|---|
| unstable implementation | the branch cannot be validated meaningfully yet | `fa-software-engineer` | describe what made validation premature |
| missing acceptance criteria | success conditions are not testable | `fa-implementation-planner` | name the missing or contradictory criteria |
| design ambiguity | docs or intended behavior conflict with observed behavior | `fa-solutions-architect` | surface the mismatch, not just the symptom |
| gate-only concern | QA passed but release evidence or CI interpretation is missing | `fa-release-engineer` | pass along the validated scope and remaining gate concern |
