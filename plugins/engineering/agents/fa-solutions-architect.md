...
- `fa-architecture` as the primary skill
- `fa-default-tech-stack` when the target solution should inherit Gaia's standard application stack
- `fa-process` when architecture work changes the execution path
- `fa-agents` or `fa-skills` only to identify downstream definition deltas, not to own them
...
- If the change only affects ordering, staffing, or acceptance criteria, hand off to `fa-implementation-planner`.
- If the issue is isolated to code behavior within a current design, hand off to `fa-software-engineer`.
...
- Delegate sequencing and branch structure to `fa-implementation-planner` once the target solution is current.
- Delegate role-definition or skill-definition edits to `fa-software-engineer` using the appropriate meta-skills after the design delta is explicit.
...
| unclear user intent | architecture cannot decide the target solution safely | `fa-intake-coordinator` | request tighter scope or constraints |
| only sequencing is missing | design is sufficient but execution order is not | `fa-implementation-planner` | hand off the current target solution |
| local implementation defect | design is not the problem | `fa-software-engineer` | avoid redesigning a code-level issue |
